...
"""
Query BacDive API to determine Gram stain classification for for HMP2 genus-level taxonomy.
"""
...
def normalize_genus_name(name: str) -> str:
    """
    Normalize a SILVA/QIIME-style genus string ONLY when formatted with leading brackets.

    Parameters
    ----------
    name : str
        Raw genus string extracted from 16S taxonomy.

    Returns
    -------
    str
        Canonical genus name suitable for BacDive queries.
        Returns "Unknown" if genus cannot be inferred.
    """
    ...
    # Case 1 — bracketed genus: [Clostridium] innocuum group
    if x.startswith("[") and "]" in x:
        end_idx = x.find("]")
        inner = x[1:end_idx].strip()

        return inner.capitalize() if inner else "Unknown"

    # Default — return original string unchanged
    return x


def query_bacdive_for_genus(
    input_csv: str,
    bacdive_config: str,
    output_csv: str,
    logger: CustomLogger,
    ...
    """
    Query BacDive API for Gram stain information of each genus extracted from 16S taxonomy.

    Parameters
    ----------
    input_csv : str
        Path to the CSV file containing a single column "Genus".
    bacdive_config : str
    ...
    try:
        ...
        # Load Input
        df_targets = load_dataframe_by_columns(input_csv, required_columns=["Genus"])
        total_targets = len(df_targets)

        # Per-class counts (for logging)
        logger.log_with_borders(
            level=logging.INFO,
            message=("[ Input Summary ]\n" f"▸ Total genus entries : {total_targets}"),
            border="|",
            length=120,
        )
        logger.add_divider(level=logging.INFO, length=120, border="+", fill="-")

        # Normalize genus names
        df_targets["Genus_normalized"] = df_targets["Genus"].apply(normalize_genus_name)
        ...
        for _, row in df_targets.iterrows():
            genus_name = row["Genus_normalized"]
            result = cached_gram_lookup(genus_name, client, gram_cache, logger)
            results.append(result)

        # Build Result
        df_results = pd.DataFrame(results)
        df_final = pd.concat([df_targets[["Genus"]], df_results], axis=1)
        ...
        logger.exception("Unexpected error in 'query_bacdive_for_genus()'")
        raise


# ============================== Pipeline Entry Point ==============================
def run_query_bacdive_genus(base_path: str, logger: CustomLogger, **kwargs) -> None:
    """
    Run the pipeline step for querying BacDive to retrieve Gram stain
    classification for each genus extracted from 16S microbiome taxonomy.
    ...
    **kwargs : dict
    ...
    try:
        ...
        output_csv = kwargs.get(
            "bacdive_output_csv",
            os.path.join(base_path, "data/processed/HMP2/genus_bacdive_gram.csv"),
        )
        ...
        logger.exception("Critical failure in 'run_query_bacdive_genus()'")
        raise
