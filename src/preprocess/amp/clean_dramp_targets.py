...
"""
Clean Dramp 'Targets' text into a normalized long table (text cleaning only).
"""
...
_BRACKET_MEASURE_RE = re.compile(
    r"\s*\((?:MIC|IC50|EC50|LC50|CC50|Ki|Kd|pIC50)[^)]*\)", re.IGNORECASE
)
...
def clean_target_string(target_str: str) -> str:
    ...
    # Remove bracketed MIC/IC50-like notes
    s = _BRACKET_MEASURE_RE.sub("", s)

    # Collapse multiple spaces to one
    s = _MULTI_SPACE_RE.sub(" ", s)

    # Remove leading/trailing spaces and punctuation (. , ; :)
    s = _EDGE_PUNCTUATION_RE.sub("", s)
...
def split_and_clean_targets_df(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Clean and 2) Split by semicolon
    df2 = (
        df1.assign(Targets=df1["Targets"].astype(str).str.split(";"))
        .explode("Targets")
        .reset_index(drop=True)
    )

    # 3) Split by '##' (common separator in DRAMP text)
    df2["Targets"] = df2["Targets"].apply(clean_target_string)
    df3 = (
        df2.assign(Targets=df2["Targets"].astype(str).str.split(r"##+"))
        .explode("Targets")
        .reset_index(drop=True)
    )
    ...
    df3["Targets"] = df3["Targets"].str.strip()
...
def clean_targets_table(
    ...
    """
    Load Dramp data, keep rows with non-empty 'Targets', text-clean and split 'Targets'
    ...
    ----------
    input_path : str
        Path to the AMP table (Excel/CSV/TSV supported by loader) with columns ['DRAMP_ID', 'Target_Organism'].
        Typically: data/raw/Dramp/general_amps.xlsx
    output_path : str
    ...
    None
    """
    logger.info("/ Task: Normalize raw Dramp targets into structured long-format table")
    ...
    try:
        ...
        df = df.rename(columns={"DRAMP_ID": "ID", "Target_Organism": "Targets"})
        df["ID"] = (
            df["ID"]
            .astype(str)
            .apply(
                lambda x: (
                    "DRAMP_" + re.sub(r"^DRAMP[_-]?", "", x)
                    if x.startswith("DRAMP") and not x.startswith("DRAMP_")
                    else x
                )
            )
        )
...
def run_clean_targets(base_path: str, logger: CustomLogger, **kwargs) -> None:
    """
    Clean AMP 'Targets' from raw Dramp table and export a normalized long CSV.
    ...
    **kwargs : dict
    ...
    try:
        input_path = kwargs.get(
            "targets_input_path",
            os.path.join(base_path, "data/raw/Dramp/general_amps.xlsx"),
        )
        output_path = kwargs.get(
            "targets_output_csv",
            os.path.join(base_path, "data/processed/Dramp/targets_clean.csv"),
        )
